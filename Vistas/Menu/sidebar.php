<div id="sidebar" class="vertical-menu">
    <ul class="nav flex-column">
        <li class="nav-item">
            <a class="nav-link" href="../Freelancer/FreelancerHome.php">
                <i class="fas fa-home"></i> Dashboard
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="../Freelancer/buscarProyectos.php">
                <i class="fas fa-folder-plus"></i> Buscar Proyectos
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="#">
                <i class="fas fa-tasks"></i> Mis Proyectos
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="../Freelancer/misPropuestas.php">
                <i class="fas fa-search"></i> Mis propuestas
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="../Freelancer/perfilFreelancer.php">
                <i class="fas fa-user"></i> Mi Perfil
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="../../logout.php">
                <i class="fas fa-sign-out-alt"></i> Cerrar Sesión
            </a>
        </li>
    </ul>
</div>
